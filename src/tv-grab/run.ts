 ...
 * messages, which a run should not pause for. The drain at the end of the run
 * is what keeps them from being lost.
 */
function queueLine(stream: Writable, line: string): void {
  stream.write(`${line}\n`);
}
 ...
 *
 * On POSIX a piped stdout is asynchronous, so `write()` alone leaves the bytes
 * queued — `process.exit()` would then discard them, and a slow reader would
 * get no backpressure. The guide and the documents go through `writeOutput`,
 * which awaits; this is what gives the rest of stdout the same guarantee.
 */
function writeFlushed(stream: Writable, text: string): Promise<void> {
 ...
 *
 * Everything the protocol puts on stdout outside a document is a list of
 * lines — one capability per line, two version lines, one description — and
 * every message on stderr is one line, so the terminator is never the caller's
 * decision to make.
 */
function writeLines(stream: Writable, ...lines: string[]): Promise<void> {
