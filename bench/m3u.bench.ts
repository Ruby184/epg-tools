...
test(
  `parse M3U (${sizeKiB} KiB, ${entries.length} entries)`,
  { timeout: TIMEOUT },
  async ({ annotate, bench }) => {
    const results = await bench.compare(
      ...tracked(bench, 'epg-tools parseM3uStream (whole string)', async () => {
        for await (const _event of parseM3uStream([playlist])) {
          // consume
        }
      }),
      ...tracked(bench, 'epg-tools parseM3uStream (64 KiB chunks)', async () => {
        for await (const _event of parseM3uStream(rechunk(playlist, 65_536))) {
          // consume
        }
      }),
      ...tracked(bench, 'epg-tools parseM3uString (sync, whole document)', () => {
        parseM3uString(playlist);
      }),
      ...tracked(bench, 'epg-tools M3uParseStream (Node Transform, 64 KiB chunks)', async () => {
        await pipeline(
    ...
    await speedup(annotate, results, 'epg-tools parseM3uStream (whole string)', [
      '@iptv/playlist parseM3U',
      'iptv-playlist-parser parse',
      'm3u-parser-generator parse',
    ]);
  },
);
...
test(`write M3U (${entries.length} entries)`, { timeout: TIMEOUT }, async ({ annotate, bench }) => {
  const results = await bench.compare(
    ...tracked(bench, 'epg-tools writeM3uStream', async () => {
      for await (const _chunk of writeM3uStream({ entries }, { eol: '\r\n' })) {
        // consume
      }
    }),
    ...tracked(bench, 'epg-tools serializeM3uEntry (per entry)', () => {
  ...
  await speedup(annotate, results, 'epg-tools writeM3uStream', [
    'm3u-parser-generator M3uGenerator',
  ]);
});
